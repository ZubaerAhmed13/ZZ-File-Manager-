...
fun HomeScreen(
    ...
    fun requestBroadAccess() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            ...
            broadSettingsLauncher.launch(intent)
        } else {
    ...
    Scaffold(
        ...
        if (state.loading) {
            Column(Modifier.padding(padding).fillMaxWidth(), verticalArrangement = Arrangement.Center) { CircularProgressIndicator(Modifier.padding(32.dp)) }
        } else {
            LazyColumn(Modifier.padding(padding)) {
                if (!state.broadStorageAccess) {
                    item {
                        PermissionCard(onGrant = ::requestBroadAccess)
                    }
                }
                item { SectionTitle(stringResource(R.string.storage)) }
                items(state.storageLocations, key = { it.id }) { storage -> StorageCard(storage, onOpenLocation) }
                item {
                    Button(
                ...
                if (state.recentLocations.isNotEmpty()) {
                    ...
                    items(state.recentLocations, key = { it.identity }) { location ->
                        ...
                        TextButton(onClick = { onOpenLocation(location) }, modifier = Modifier.padding(start = 56.dp)) { Text(location.displayName) }
                    }
                }
...
private fun PermissionCard(onGrant: () -> Unit) {
    Card(Modifier.fillMaxWidth().padding(16.dp)) {
        Column(Modifier.padding(16.dp)) {
            Row { Icon(Icons.Default.Storage, contentDescription = null); Spacer(Modifier.padding(4.dp)); Text(stringResource(R.string.full_storage_access), style = MaterialTheme.typography.titleMedium) }
...
private fun StorageCard(storage: StorageLocation, onOpen: (BrowserLocation) -> Unit) {
    Card(onClick = { onOpen(storage.root) }, modifier = Modifier.fillMaxWidth().padding(horizontal = 16.dp, vertical = 6.dp)) {
        Column(Modifier.padding(16.dp)) {
            Row { Icon(Icons.Default.Storage, contentDescription = null); Spacer(Modifier.padding(4.dp)); Text(storage.displayName, style = MaterialTheme.typography.titleMedium) }
            val total = storage.totalBytes
            val used = storage.usedBytes
            val free = storage.freeBytes
            if (total != null && used != null && total > 0L) {
                Text(stringResource(R.string.used_of_total, Formatters.bytes(used), Formatters.bytes(total)), style = MaterialTheme.typography.bodySmall, modifier = Modifier.padding(top = 8.dp))
                LinearProgressIndicator(progress = { (used.toDouble() / total.toDouble()).toFloat().coerceIn(0f, 1f) }, modifier = Modifier.fillMaxWidth().padding(vertical = 6.dp))
...
@Composable
private fun SectionTitle(text: String) { Text(text, style = MaterialTheme.typography.titleLarge, modifier = Modifier.padding(start = 16.dp, top = 20.dp, end = 16.dp, bottom = 8.dp)) }

@Composable
private fun CategoryButton(category: MediaCategory, labelRes: Int, icon: androidx.compose.ui.graphics.vector.ImageVector, onClick: (MediaCategory) -> Unit) {
