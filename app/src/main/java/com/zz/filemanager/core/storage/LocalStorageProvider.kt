...
class LocalStorageProvider(private val context: Context) : WritableStorageProvider {
    ...
    override suspend fun capabilities(location: BrowserLocation): ProviderCapabilities = withContext(Dispatchers.IO) {
        ...
        if (directory.canWrite() && location.writable) {
            capabilities += setOf(
                ...
                StorageCapability.RANDOM_ACCESS,
                StorageCapability.ATOMIC_RENAME,
                StorageCapability.SET_MODIFIED_TIME,
            )
        }
    ...
    override suspend fun createDirectory(parent: BrowserLocation, name: String): FileEntry = withContext(Dispatchers.IO) {
        ...
        val directory = validated(parent.reference, parent.rootReference)
        if (!directory.canWrite()) throw StorageAccessException.ReadOnly()
        val child = File(directory, name)
        if (child.exists()) throw StorageAccessException.Io(IllegalStateException("destination exists"))
        runCatching { Files.createDirectory(child.toPath()) }
            .getOrElse { throw mapWriteFailure(it) }
        toEntry(child, parent.storageId)
    }
    ...
    override suspend fun rename(item: ScopedFileReference, newName: String): FileEntry = withContext(Dispatchers.IO) {
        ...
        if (!source.exists()) throw StorageAccessException.Unavailable()
        if (source.name == newName) return@withContext toEntry(source, item.storageId)
        val parent = source.parentFile ?: throw StorageAccessException.Io()
        val target = File(parent, newName)
        if (!isInside(target.canonicalFile, File(item.rootReference).canonicalFile)) throw StorageAccessException.PermissionRequired()

        val sameUnderlying = target.exists() && runCatching { target.canonicalFile == source.canonicalFile }.getOrDefault(false)
        if (target.exists() && !sameUnderlying) throw StorageAccessException.Io(IllegalStateException("destination exists"))

        try {
            if (sameUnderlying) {
                val temporary = uniqueTemporarySibling(parent, ".zzcase-${UUID.randomUUID()}")
                Files.move(source.toPath(), temporary.toPath())
                try {
                    Files.move(temporary.toPath(), target.toPath())
                } catch (error: Throwable) {
                    runCatching { Files.move(temporary.toPath(), source.toPath()) }
                    throw error
                }
            } else {
    ...
    override suspend fun openOutputStream(item: ScopedFileReference, truncate: Boolean): OutputStream = withContext(Dispatchers.IO) {
        val file = scoped(item)
        if (!file.exists()) throw StorageAccessException.Unavailable()
        if (!file.isFile) throw StorageAccessException.Io()
        try {
            FileOutputStream(file, !truncate)
        } catch (error: SecurityException) {
            throw StorageAccessException.PermissionRequired(error)
        } catch (error: Throwable) {
            throw StorageAccessException.Io(error)
        }
    }
    ...
    override suspend fun findChild(parent: BrowserLocation, name: String): FileEntry? = withContext(Dispatchers.IO) {
        ...
        val directory = validated(parent.reference, parent.rootReference)
        val child = File(directory, name)
        if (!child.exists()) null else toEntry(child, parent.storageId)
    }

    override suspend fun freeBytes(location: BrowserLocation): Long? = withContext(Dispatchers.IO) {
        val directory = validated(location.reference, location.rootReference)
        directory.usableSpace.takeIf { it >= 0L }
    }
    ...
    override suspend fun moveNative(
        ...
        if (!destinationDirectory.canWrite()) throw StorageAccessException.ReadOnly()
        val target = File(destinationDirectory, newName)
        if (target.exists()) return@withContext null

        val sameFileStore = runCatching {
            Files.getFileStore(source.toPath()) == Files.getFileStore(destinationDirectory.toPath())
        }.getOrDefault(false)
        ...
        try {
            ...
            toEntry(target, destination.storageId)
        } catch (error: Throwable) {
            throw mapWriteFailure(error)
        }
    }
    ...
    private fun toEntry(file: File, storageId: String): FileEntry {
        val symbolicLink = runCatching { Files.isSymbolicLink(file.toPath()) }.getOrDefault(false)
        ...
        val type = if (symbolicLink && file.isDirectory) {
            com.zz.filemanager.core.model.FileEntryType.GENERIC
        } else {
            FileClassifier.classify(file.name, mime, file.isDirectory)
        }
        ...
        return FileEntry(
            ...
            mimeType = mime,
            type = type,
            sizeBytes = if (file.isFile || symbolicLink) file.length() else null,
            ...
            childCount = null,
            storageId = storageId,
            thumbnailKey = if (file.isFile && !symbolicLink) "${file.absolutePath}:${modified ?: 0L}:${file.length()}" else null,
            isSymbolicLink = symbolicLink,
        )
    }

    private fun scoped(item: ScopedFileReference): File {
        val path = item.reference.path ?: throw StorageAccessException.Unavailable()
        return validated(path, item.rootReference)
    }
    ...
    private fun uniqueTemporarySibling(parent: File, prefix: String): File {
        var index = 0
        while (true) {
            val candidate = File(parent, if (index == 0) prefix else "$prefix-$index")
            if (!candidate.exists()) return candidate
            index++
        }
    }

    private fun mapWriteFailure(error: Throwable): StorageAccessException = when (error) {
