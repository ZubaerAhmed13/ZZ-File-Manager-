...
class ThumbnailRepository(private val context: Context) {
    ...
    suspend fun load(entry: FileEntry, width: Int, height: Int): Bitmap? {
        ...
        return withContext(Dispatchers.IO) {
            ...
            val bitmap = runCatching { decode(entry, width, height) }.getOrNull()
            coroutineContext.ensureActive()
    ...
    private fun decode(entry: FileEntry, width: Int, height: Int): Bitmap? {
        ...
        return when (entry.type) {
            FileEntryType.IMAGE -> decodeImage(entry, width, height)
            FileEntryType.VIDEO -> decodeVideo(entry, width, height)
            else -> null
        }
    }
    ...
    private fun decodeImage(entry: FileEntry, width: Int, height: Int): Bitmap? {
        ...
        val decoded = if (path != null) BitmapFactory.decodeFile(path, options) else uri?.let { context.contentResolver.openInputStream(it)?.use { stream -> BitmapFactory.decodeStream(stream, null, options) } }
            ?: return null
        val orientation = runCatching {
            val exif = if (path != null) ExifInterface(path) else uri?.let { context.contentResolver.openInputStream(it)?.use(::ExifInterface) }
    ...
    private fun decodeVideo(entry: FileEntry, width: Int, height: Int): Bitmap? {
        val path = entry.reference.path
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q && path != null) {
            return ThumbnailUtils.createVideoThumbnail(File(path), Size(width, height), null)
        }
        val retriever = MediaMetadataRetriever()
        return try {
            if (path != null) retriever.setDataSource(path) else entry.reference.uri?.let(Uri::parse)?.let { retriever.setDataSource(context, it) } ?: return null
            val frame = retriever.getFrameAtTime(0, MediaMetadataRetriever.OPTION_CLOSEST_SYNC) ?: return null
            val scale = minOf(width.toFloat() / frame.width, height.toFloat() / frame.height, 1f)
            if (scale >= 1f) frame else Bitmap.createScaledBitmap(frame, max(1, (frame.width * scale).toInt()), max(1, (frame.height * scale).toInt()), true).also { if (it !== frame) frame.recycle() }
